...
(() => {
  ...
  chrome.runtime.onMessage.addListener(
    (message, _sender, sendResponse) => {
      if (message.type === "FETCH_PRICES") {
        handleFetchPrices(message.query, message.productType).then((data) => sendResponse({ ok: true, data })).catch((err) => sendResponse({ ok: false, error: err.message }));
        return true;
      }
      if (message.type === "TRACK_PURCHASE") {
        handleTrackPurchase(message.purchase).then(() => sendResponse({ ok: true }));
        return true;
      }
  ...
  async function handleFetchPrices(query, productType) {
    const result = { ebay: null };
    try {
