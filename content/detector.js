...
(() => {
  ...
  `;
    ...
    function buildCompareLinks(product) {
      ...
      return links.map(
        (l) => `<a class="pw-link" href="${l.url}" target="_blank" rel="noopener noreferrer">${l.label}</a>`
      ).join("");
    }
    function buildHTML(product) {
      ...
      <div class="pw-panel pw-hidden" id="pw-panel">
        <div class="pw-header">
          <span class="pw-logo">\u{1F0CF} PackWatch</span>
          <div class="pw-header-btns">
            <button class="pw-btn" id="pw-min" title="Minimise">\u2212</button>
            <button class="pw-btn" id="pw-close" title="Dismiss">\xD7</button>
          </div>
        </div>
    ...
    function wireEvents(root, product) {
      ...
      const trackBtn = root.querySelector("#pw-track");
    ...
    function loadPrices(product) {
      const query = product.productName.replace(/\|.*/, "").replace(/\s*[-–].*(?:ebay|amazon|tcg|shop|store|buy).*/i, "").trim().slice(0, 100);
      chrome.runtime.sendMessage(
        ...
        (res) => {
          ...
          if (res?.ok && res.data) {
            ...
            el.innerHTML = '<div class="pw-no-data">Live prices unavailable \xB7 use links below</div>';
          }
        }
      );
    }
